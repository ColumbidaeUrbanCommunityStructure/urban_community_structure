...
write_csv(pools %>% 
            dplyr::select(
              ...
              present_urban_med,
              present_urban_low
            ), filename(COMMUNITY_OUTPUT_DIR, 'communities_for_analysis.csv'))
...
abundance_data_jetz$max_mean_abundance[abundance_data_regional$max_mean_abundance == -Inf] = NA
```
...
comparison_data %>% 
  ...
  filter(value) %>%
  ggplot(aes(x = urban_pool, y = max_mean_abundance)) + geom_boxplot() + xlab('Threshold') + ylab('Mean abundance across cities') +
