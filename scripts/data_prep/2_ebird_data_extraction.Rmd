...
Clean up by removing two large collections of all ebird observations
```{r}
...
SELECT checklist_id, locality_id, duration, effort_km, obs_date 
FROM dedupe_group 
WHERE row_number = 1
UNION ALL
SELECT checklist_id, locality_id, duration, effort_km, obs_date 
...
```{bash}
echo -e ".mode csv\n.headers on\nSELECT checklist_id, locality_id, city_id, city_name, duration, obs_date FROM unique_checklists JOIN locality_to_city USING (locality_id) WHERE duration >= 5 AND duration <= 240 AND (effort_km = '' OR (effort_km >= 0 AND effort_km <= 10))" > /tmp/export_command.sqlite
...
```{r}
EBIRD_DATA_RAW = '/Users/james/Dropbox/PhD/eBird/ebd_relNov-2023/ebd_relNov-2023.sample.txt'
