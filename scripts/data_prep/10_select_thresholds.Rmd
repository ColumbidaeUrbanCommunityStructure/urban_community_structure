...
```{r}
pools = read_csv(filename(COMMUNITY_OUTPUT_DIR, 'recorded_species_with_spatial_data.csv'))
pools
```
```{r}
...
```{r}
reported_rows = c('city_id', 'city_name', 'ebird_species_name', 'percentage_checklists', 'percentage_locations', 'total_city_checklists', 'total_city_locations', 'buf_500m_percentage_survey_area_present', 'buf_500m_percentage_total_city_area_surveyed')
```
...
Columba_fasciata = pools %>% 
  filter(ebird_species_name %in% c('Patagioenas fasciata')) %>% 
...
Columba_livia = pools %>% 
  filter(ebird_species_name %in% c('Columba livia')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columba_livia
```
...
Columba_palumbus = pools %>% 
  filter(ebird_species_name %in% c('Columba palumbus')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columba_palumbus
```
...
Columba_picazuro = pools %>% 
  filter(ebird_species_name %in% c('Patagioenas picazuro')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columba_picazuro
```
...
Columbina_cruziana = pools %>% 
  filter(ebird_species_name %in% c('Columbina cruziana')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columbina_cruziana
```
...
Columbina_inca = pools %>% 
  filter(ebird_species_name %in% c('Columbina inca')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columbina_inca
```
...
Columbina_passerina = pools %>% 
  filter(ebird_species_name %in% c('Columbina passerina')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columbina_passerina
```
...
Columbina_squammata = pools %>% 
  filter(ebird_species_name %in% c('Columbina squammata')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columbina_squammata
```
...
Columbina_talpacoti = pools %>% 
  filter(ebird_species_name %in% c('Columbina talpacoti')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Columbina_talpacoti
```
...
Streptopelia_chinensis = pools %>% 
  filter(ebird_species_name %in% c('Spilopelia chinensis')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
...
Streptopelia_decaocto = pools %>% 
  filter(ebird_species_name %in% c('Streptopelia decaocto')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
...
Streptopelia_lugens = pools %>% 
  filter(ebird_species_name %in% c('Streptopelia lugens')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
Streptopelia_lugens
```
...
Streptopelia_orientalis = pools %>% 
  filter(ebird_species_name %in% c('Streptopelia orientalis')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
...
Streptopelia_semitorquata = pools %>% 
  filter(ebird_species_name %in% c('Streptopelia semitorquata')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
...
Streptopelia_senegalensis = pools %>% 
  filter(ebird_species_name %in% c('Spilopelia senegalensis')) %>% 
  filter(city_name %in% cities) %>% 
  dplyr::select(reported_rows)
...
```{r}
all_species %>% 
  filter(buf_500m_percentage_total_city_area_surveyed > 10) %>%
  summarise(
    number_records = n(),
    mean_percentage_surveyed_area = mean(buf_500m_percentage_survey_area_present),
    lower_10_quant_percentage_surveyed_area = quantile(buf_500m_percentage_survey_area_present, 0.1),
    lower_20_quant_percentage_surveyed_area = quantile(buf_500m_percentage_survey_area_present, 0.2),
    min_percentage_surveyed_area = min(buf_500m_percentage_survey_area_present),
    mean_percentage_checklists = mean(percentage_checklists),
    lower_10_quant_percentage_checklists = quantile(percentage_checklists, 0.1),
    lower_20_quant_percentage_checklists = quantile(percentage_checklists, 0.2),
    min_percentage_checklists = min(percentage_checklists)
