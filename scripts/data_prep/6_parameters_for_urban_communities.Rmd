...
## Check how this species are mapped from eBird to Birdlife
```{r}
test_species_mapping = taxonmomic_mapping[taxonmomic_mapping$ebird_species_fullname %in% test_species,]
...
urban_ebird_columbidae_city_summary = ebird_raw %>% 
  left_join(taxonmomic_mapping, by = c('species_name' = 'ebird_species_fullname')) %>% 
  group_by(city_id, city_name, ebird_species_name) %>% 
