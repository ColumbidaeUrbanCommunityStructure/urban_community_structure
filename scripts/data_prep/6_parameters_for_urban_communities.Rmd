...
get_mean_test_error = function(min_city_checklists, get_model, get_expected) {
  ...
  for(seed in c(123, 456, 678, 10, 11, 345, 32, 11, 54, 90, 9999, 1234, 5678, 2323, 9011, 532, 111, 678, 6501, 3)) {
    set.seed(seed)
...
for (min_city_checklist in min_city_checklists) {
  result = rbind(result, data.frame(
    ...
    mean_test_error = get_mean_test_error(
      min_city_checklist, 
      function(training_data) {glm((max_abundance > 0) ~ percentage_checklists, binomial, training_data)},
      function(test_data) {test_data$max_abundance > 0})
  ))
}
