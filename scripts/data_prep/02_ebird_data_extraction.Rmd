...
@eBird2023 data:
```{r}
Sys.setenv(EBIRD_SAMPLE_FILE = DL_EBIRD_SAMPLE_DATA_RAW)
```
...
```{bash}
echo '.separator \t' > /tmp/import_command.sqlite
echo '.import ${EBIRD_CHECKLIST_FIRST_PASS_FILE} checklists' >> /tmp/import_command.sqlite
...
```{r}
taxonomy = read_csv(DL_EBIRD_TAXONOMY)
head(taxonomy)
```
...
Sys.setenv(EBIRD_FILE = DL_EBIRD_DATA_RAW)
```
