...
output: 
  html_document:
    ...
    biblio-style: apsr
references:
  - id: GHS_SMOD_POPMT_GLOBE_R2019A
...
```{r}
nrow(human_settlement_2mil)
```

Location of those cities
```{r}
plot(human_settlement_2mil["pop_2015"])
```

## Store cities for use later.
```{r}
output = mkdir(GEO_DATA_OUTPUT_DIR, 'cities')
write_sf(human_settlement_2mil, filename(output, 'initial_selection.shp'), driver="ESRI Shapefile", layer_options = "ENCODING=UTF-8")
```
