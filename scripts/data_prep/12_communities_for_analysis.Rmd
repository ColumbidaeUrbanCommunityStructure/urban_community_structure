...
```{r}
distances = read_csv(filename(GEO_WORKING_OUTPUT_DIR, 'distance_to_range_edge_v2.csv'))
nrow(pools)
...
write_csv(pools %>%
            left_join(distances) %>% 
            mutate(distance_to_range_edge_km = distance / 1000) %>%
            dplyr::select(
              city_id,
              city_name,
              jetz_species_name,
