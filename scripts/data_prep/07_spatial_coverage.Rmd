...
surveyed_area = function(checklists_sf, city_polygon, buffer_size) {
  checklists_sf %>%
...
for (selected_city_id in city_total_areas$city_id) {
  ...
  if (nrow(checklist_locations) > 0) {
    ...
    city_total_areas$surveyed_area_250m[city_total_areas$city_id == selected_city_id] = surveyed_area(checklists_sf, city_polygon, RADIUS_FROM_CHECKLIST_CENTRE_FOR_SPATIAL_COVERAGE_250M)
    city_total_areas$surveyed_area_500m[city_total_areas$city_id == selected_city_id] = surveyed_area(checklists_sf, city_polygon, RADIUS_FROM_CHECKLIST_CENTRE_FOR_SPATIAL_COVERAGE_500M)
    city_total_areas$surveyed_area_1km[city_total_areas$city_id == selected_city_id] = surveyed_area(checklists_sf, city_polygon, RADIUS_FROM_CHECKLIST_CENTRE_FOR_SPATIAL_COVERAGE_1KM)
...
Create shape file of all buffered urban checklists
```{r}
urban_checklists_sf = urban_checklists %>%
    st_as_sf(coords = c("long", "lat"))

st_crs(urban_checklists_sf) = 4326

urban_checklists_sf %>%
    st_transform(METRE_BASED_CRS) %>%
    st_buffer(RADIUS_FROM_CHECKLIST_CENTRE_FOR_SPATIAL_COVERAGE_500M) %>%
    st_union() %>%
    st_write(filename(filename(GEO_WORKING_OUTPUT_DIR, 'ebird'), 'urban_checklists.shp'))
```
...
```{r}
ggpairs(pools_city_data[,c('buf_250m_percentage_survey_area_present', 'buf_500m_percentage_survey_area_present', 'buf_1km_percentage_survey_area_present')], columnLabels = c('250m buffer', '500m buffer', '1km buffer'), upper = list(continuous = wrap(ggally_cor, method = "spearman")))
