...
```{r}
mapping_birdlife_to_ebird= unique(taxonomic_mapping[,c('species_name', 'ebird_species_name')])
mapping_birdlife_to_ebird
```
...
regional_pool_data_seasonal = regional_pool_data_input %>% 
  left_join(mapping_birdlife_to_ebird) %>%
  arrange(seasonal) %>%
...
```{r}
ebird_species_name_to_full_species_name = distinct(taxonomic_mapping[,c('ebird_species_fullname', 'ebird_species_name')])

urban_columbidae_presence_records = ebird_raw %>% 
  rename(ebird_species_fullname = species_name) %>%
  left_join(urban_checklists[,c('checklist_id', 'locality_id')]) %>%
  left_join(ebird_species_name_to_full_species_name) %>%
  group_by(city_id, city_name, ebird_species_name) %>% 
...
```{r}
urban_columbidae_presence_records[urban_columbidae_presence_records$city_name %in% inspection_cities,]
```
...
```{r}
urban_columbidae_presence_and_absence = rbind(urban_columbidae_presence_records, urban_columbidae_absence_records) %>%
  group_by(city_id, city_name, ebird_species_name, total_city_checklists, total_city_locations, total_city_effort, seasonal, presence, origin) %>%
...
```{r}
write_csv(urban_columbidae_presence_and_absence, filename(COMMUNITY_OUTPUT_DIR, 'recorded_species.csv'))
```
...
data.frame(urban_columbidae_presence_and_absence) %>%
  ...
  dplyr::select(city_id, city_name, ebird_species_name, percentage_checklists, total_city_checklists) %>%
