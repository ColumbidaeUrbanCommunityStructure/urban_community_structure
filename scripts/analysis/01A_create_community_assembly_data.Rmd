...
## Test communities
```{r}
test_communities = to_species_matrix_with_relative_abundance(c(79, 624, 1786, 2059, 11536))
test_communities
```
...
create_null_model_communities = function(city_id, urban_richness, regional_species, proxy_relative_abundance_values) {
  ...
  null_model_sites = expand.grid(rep(list(proxy_relative_abundance_values), regional_richness)) %>% mutate(richness = rowSums(. != 0))
  null_model_sites = null_model_sites[null_model_sites$richness == urban_richness,]
...
```
```{r}
test_null_model_city_1 = create_null_model_communities(
  1, 
