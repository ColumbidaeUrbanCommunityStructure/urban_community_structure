...
## Kipps distance - FDiv
```{r}
norm_kipps_fdiv_loco_analysis_plot = geom_normalised_histogram(
  'Kipps distance FDiv', 
  ggplot(community_data_with_realm, aes(kipps_distance_fdiv_normalised))
)
norm_kipps_fdiv_loco_analysis_plot
```

```{r}
norm_kipps_fdiv_analysis_geo = geom_map(geom_sf(data = city_points, aes(color = kipps_distance_fdiv_normalised, geometry = geometry)), 'Kipps Distance FDiv')
norm_kipps_fdiv_analysis_geo
ggsave(filename(FIGURES_OUTPUT_DIR, 'normalised_kipps_distance_using_abundance.jpg'), width = 2500, height=1100, units = 'px')
```

```{r}
summary(lm(data = city_points, kipps_distance_fdiv_normalised ~ latitude))
```
```{r}
summary(lm(data = city_points, kipps_distance_fdiv_normalised ~ abs(latitude)))
```
...
```{r}
ggplot(community_data_with_realm, aes(x = kipps_distance_fdiv_normalised, y = mntd_normalised, colour = core_realm)) + 
  geom_point() +
  ylab("MNTD") + 
  xlab("Kipps Distance FDiv") +
  theme_bw() + labs(color = "Realm")
```
```{r}
ggplot(community_data_with_realm, aes(x = kipps_distance_fdiv_normalised, y = gape_width_fdiv_normalised, colour = core_realm)) + 
  geom_point() +
...
mntd_fdiv_analysis = community_data_with_realm %>% 
  dplyr::select(city_id,  mntd_normalised, kipps_distance_fdiv_normalised, gape_width_fdiv_normalised) %>%
...
```{r}
ggpairs(mntd_fdiv_analysis %>% dplyr::select(mntd_normalised, kipps_distance_fdiv_normalised, gape_width_fdiv_normalised, regional_pool_size, urban_pool_size, urban_pool_perc))
```
