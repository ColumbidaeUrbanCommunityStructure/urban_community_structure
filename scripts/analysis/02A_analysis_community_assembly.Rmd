...
all_explanatories = c(
    ...
    'region_50km_min_elev', 'region_50km_elev_range',
    'core_realmAfrotropic', 'core_realmAustralasia', 'core_realmIndomalayan', 'core_realmNearctic', 'core_realmNeotropic')
...
plot_vsurp_result = function(result_table) {
  ...
  ggplot(plot, aes(y=explanatory, x=estimate, colour = type)) + 
    ...
    guides(colour=guide_legend(title="Predictor type")) + xlab('Increase from 0 (habitat filtering) to 1 (competitive exclusion)\n± Standard Error') + ylab('Predictor') +
...
norm_mntd_analysis_result <- model_average(norm_mntd_analysis_formula, norm_mntd_analysis_data, 10)
norm_mntd_analysis_result_table = model_summary(norm_mntd_analysis_result)
norm_mntd_analysis_result_table
```

```{r}
plot_vsurp_result(norm_mntd_analysis_result_table)
```
...
norm_gape_fdiv_analysis_result <- model_average(norm_gape_fdiv_analysis_formula, norm_gape_fdiv_analysis_data, 10)
norm_gape_fdiv_analysis_result_table = model_summary(norm_gape_fdiv_analysis_result)
norm_gape_fdiv_analysis_result_table
```

```{r}
plot_vsurp_result(norm_gape_fdiv_analysis_result_table)
```
...
norm_hwi_fdiv_analysis_result <- model_average(norm_hwi_fdiv_analysis_formula, norm_hwi_fdiv_analysis_data, 10)
norm_hwi_fdiv_analysis_result_table = model_summary(norm_hwi_fdiv_analysis_result)
norm_hwi_fdiv_analysis_result_table
```

```{r}
plot_vsurp_result(norm_hwi_fdiv_analysis_result_table)
```
...
norm_mass_fdiv_analysis_result <- model_average(norm_mass_fdiv_analysis_formula, norm_mass_fdiv_analysis_data, 10)
norm_mass_fdiv_analysis_result_table = model_summary(norm_mass_fdiv_analysis_result)
norm_mass_fdiv_analysis_result_table
```

```{r}
plot_vsurp_result(norm_mass_fdiv_analysis_result_table)
```
