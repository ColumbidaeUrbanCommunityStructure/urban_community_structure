...
plot_spatial_result = function(model_result) {
  ...
  ggplot2::ggplot(result_table, ggplot2::aes(y=factor(explanatory_name, level = all_explanatory_names, ordered = T), x=Value, colour = type)) + 
    ...
    scale_colour_manual(
      ...
      breaks = c('Realm', 'City geography', 'Regional (50km) geography', 'Introduced species')) +
...
```{r}
mntd_spatial_model = spatial_model(std_mntd_analysis_formula, corLin(form = correlation_formula))
```
...
```{r}
std_gape_fdiv_spatial_model = spatial_model(std_gape_fdiv_analysis_formula, corLin(form = correlation_formula))
```
...
```{r}
std_hwi_fdiv_spatial_model = spatial_model(std_hwi_fdiv_analysis_formula, corLin(form = correlation_formula))
```
...
```{r}
std_mass_fdiv_spatial_model = spatial_model(std_mass_fdiv_analysis_formula, corLin(form = correlation_formula))
```
